  ...
  return (
    <div className="pt-32 px-6 pb-20 max-w-5xl mx-auto text-white animate-[fadeIn_0.5s_ease-out]">
        ...
        <div className="p-6 bg-white/5 rounded-xl border border-white/10 mb-8">
            <h3 className="text-xl font-bold mb-2 text-sfc-orange truncate">Welcome, {user?.name || user?.email}</h3>
            ...
            {user?.membership?.plan === 'free' && (
                <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded inline-block">
                    <p className="text-yellow-400 text-sm font-bold">Free Trial Remaining: {user?.membership?.trialRemaining}</p>
                </div>
            )}
            <div className="mt-6 flex gap-4">
              <button onClick={() => { logout(); onNavigate('home'); }} className="px-5 py-2 bg-red-500/20 text-red-400 font-bold rounded-lg hover:bg-red-500/40 transition-colors">Logout</button>
              <button onClick={() => onNavigate('aiSaas')} className="px-5 py-2 bg-sfc-blue text-white font-bold rounded-lg hover:bg-blue-600 transition-colors">Upgrade / Go to AI</button>
            </div>
        </div>
